...
// console.log(state())		//[ 'Madhya Pradesh', 'Jharkhand', 'Assam', 'Bihar' ]

function city(stateName) {
	const cityObj = states.find(item => item.name == stateName)
	const cityArray = cityObj.city
	return cityArray.map(item => (item.name))
}

// console.log(city("Assam"))		//[ 'Guwhati', 'dimapur', 'Tezpur' ]

function landmark(state, cityName) {
	const selectedState = states.find(item => item.name == state)

	// const selectedCity = selectedState.city.find(item=>item.city==cityName)
	const stateData = selectedState.city
	const selectedCity = stateData.find(item => item.name == cityName)
	// console.log(selectedCity)

	const landmarkData = selectedCity.landmarks
	return landmarkData.map(item => item.name)
}

// landmark("Assam","Guwhati")
// console.log(landmark("Assam", "Guwhati"));		//[ 'Ganesh Guri', 'Kalyani Nagar' ]


export function stateDescription(currState) {

const selectedObject=	states.find(item => item.name == currState)
return selectedObject
//fetch description from selectedObject
}

// console.log( stateDescription("Madhya Pradesh").name)
export function cityDescription(currState,currCity){
	const stateDesc = stateDescription(currState)
	// console.log(stateDesc)
	return stateDesc.city.find(city=> city.name == currCity)
}
console.log(cityDescription("Madhya Pradesh","Bhopal").name)

// export function landmarkDescription(currState,currCity,currLandmark){
// const cityDesc = cityDescription(currState,currCity)
// return cityDesc.landmarks.find(landmark=> landmark.name == currLandmark)
// }
export function landmarkDescription(state, city, landmark) {
	const stateData = states.find(s => s.name === state);
	const cityData = stateData?.city?.find(c => c.name === city);
	return cityData?.landmarks?.find(l => l.name === landmark) || {};
  }

export { state, city, landmark }